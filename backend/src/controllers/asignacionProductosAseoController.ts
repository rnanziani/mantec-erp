...
import {
  AsignacionProductoAseo,
  DetalleAsignacionProducto,
  ProductoAseo,
  CreateAsignacionProductoAseoDTO,
  UpdateAsignacionProductoAseoDTO,
  CreateProductoAseoDTO,
  UpdateProductoAseoDTO,
  ApiResponse
} from '../types.js';
...
export const getAllAsignaciones = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query<AsignacionProductoAseo>(query, params);

    const response: ApiResponse<AsignacionProductoAseo[]> = {
      success: true,
      data: result.rows
    };
...
export const getAsignacionById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query<AsignacionProductoAseo>(query, [id]);
...
export const getDetallesAsignacion = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const response: ApiResponse<DetalleAsignacionProducto[]> = {
      success: true,
      data: result.rows
    };
...
export const createAsignacion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const {
      ...
      hora,
      detalles
    }: CreateAsignacionProductoAseoDTO = req.body;
    ...
    const asignacionResultFinal = await client.query<AsignacionProductoAseo>(
      asignacionQuery,
      [idAsignacion]
    );
...
export const updateAsignacion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const {
      ...
      hora,
      detalles
    }: UpdateAsignacionProductoAseoDTO = req.body;
    ...
    const result = await client.query<AsignacionProductoAseo>(asignacionQuery, [id]);
...
export const deleteAsignacion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result = await client.query(
      `DELETE FROM ${TABLA_ASIGNACION} WHERE idproductomain_12 = $1 RETURNING idproductomain_12`,
      [id]
    );
...
export const getReporteDatos = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (id_trabajador) {
      ...
      params.push(parseInt(id_trabajador as string));
      paramCount++;
    }

    if (id_producto) {
      query += ` AND pd.idproductoaseo_13 = $${paramCount}`;
      params.push(parseInt(id_producto as string));
      paramCount++;
    }
...
export const generarReportePDF = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    filtrosAplicados.push(`Período: ${formatDate(fecha_desde as string)} - ${formatDate(fecha_hasta as string)}`);
    ...
    if (id_trabajador) {
      const trabajadorResult = await pool.query(
        `SELECT nombre_06, apaterno_06, amaterno_06 FROM ${TABLA_TRABAJADOR} WHERE idtrabajador_06 = $1`,
        [id_trabajador]
      );
    ...
    if (id_producto) {
      const productoResult = await pool.query(
        `SELECT productoaseo_10 FROM ${TABLA_PRODUCTO} WHERE idproductoaseo_10 = $1`,
        [id_producto]
      );
