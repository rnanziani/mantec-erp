...
import { OrdenTrabajo, CreateOrdenTrabajoDTO, UpdateOrdenTrabajoDTO } from '../types.js';

/**
 * Obtener todas las órdenes de trabajo con JOINs
 */
export const getAllOrdenes = async (req: Request, res: Response): Promise<void> => {
    try {
        const query = `
      SELECT 
        ot.id_orden_24,
    ...
    `;
        ...
        const result = await pool.query<OrdenTrabajo>(query);
 ...
 */
export const getOrdenById = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const query = `
      SELECT 
    ...
    `;
        ...
        const result = await pool.query<OrdenTrabajo>(query, [id]);
...
export const createOrden = async (req: Request, res: Response): Promise<void> => {
    try {
        const {
            ...
            diagnostico_24,
            trabajo_realizado_24,
            costo_reparacion_24,
            tecnico_responsable_24,
            estado_orden_24
        }: CreateOrdenTrabajoDTO = req.body;
        ...
        const result = await pool.query<OrdenTrabajo>(
            `INSERT INTO tbl_24_orden_trabajo 
...
export const updateOrden = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const {
            ...
            tecnico_responsable_24,
            estado_orden_24
        }: UpdateOrdenTrabajoDTO = req.body;
...
export const deleteOrden = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const result = await pool.query(
            'DELETE FROM tbl_24_orden_trabajo WHERE id_orden_24 = $1 RETURNING id_orden_24',
            [id]
        );
