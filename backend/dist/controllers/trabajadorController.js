...
export const createTrabajador = async (req, res) => {
    try {
        ...
        const checkRut = await pool.query('SELECT idtrabajador_06 FROM tbl_06_trabajador WHERE ruttrabajador_06 = $1', [rutFormateado]);
...
export const updateTrabajador = async (req, res) => {
    try {
        ...
        const checkRut = await pool.query('SELECT idtrabajador_06 FROM tbl_06_trabajador WHERE ruttrabajador_06 = $1 AND idtrabajador_06 != $2', [rutFormateado, id]);
        if (checkRut.rows.length > 0) {
            const response = {
                success: false,
