...
export const getAllTipos = async (req, res) => {
    try {
        const result = await pool.query(`SELECT * FROM tbl_25_tipo_transaccion ORDER BY descripcion_25 ASC`);
        const response = {
            success: true,
            data: result.rows
        };
...
export const getTipoById = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query(`SELECT * FROM tbl_25_tipo_transaccion WHERE id_tipo_transaccion_25 = $1`, [id]);
...
export const createTipo = async (req, res) => {
    try {
        const { descripcion_25, cod_accion_25, valor_accion_25 } = req.body;
        // Validaciones
        ...
        const existingCode = await pool.query(`SELECT id_tipo_transaccion_25 FROM tbl_25_tipo_transaccion WHERE cod_accion_25 = $1`, [cod_accion_25]);
        ...
        const result = await pool.query(`INSERT INTO tbl_25_tipo_transaccion (descripcion_25, cod_accion_25, valor_accion_25)
       VALUES ($1, $2, $3)
       RETURNING *`, [descripcion_25.trim(), cod_accion_25, valor_accion_25]);
        const response = {
            success: true,
            data: result.rows[0],
...
export const updateTipo = async (req, res) => {
    try {
        ...
        values.push(id);
        const result = await pool.query(`UPDATE tbl_25_tipo_transaccion
       SET ${updates.join(', ')}
