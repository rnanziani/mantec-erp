...
export const createMaquina = async (req, res) => {
    try {
        ...
        const empresaCheck = await pool.query('SELECT idempresa_15 FROM tbl_15_empresas WHERE idempresa_15 = $1', [idempresa_11]);
...
export const updateMaquina = async (req, res) => {
    try {
        ...
        const maquinaCheck = await pool.query('SELECT idmaquina_11 FROM tbl_11_maquina WHERE idmaquina_11 = $1', [id]);
        ...
        if (numinterno_11 || idempresa_11) {
            const currentMaquina = await pool.query('SELECT numinterno_11, idempresa_11 FROM tbl_11_maquina WHERE idmaquina_11 = $1', [id]);
...
export const deleteMaquina = async (req, res) => {
    try {
        const { id } = req.params;
        // Verificar que la máquina existe
        const maquinaCheck = await pool.query('SELECT idmaquina_11, estado_11 FROM tbl_11_maquina WHERE idmaquina_11 = $1', [id]);
        if (maquinaCheck.rowCount === 0) {
            const response = {
                success: false,
                error: 'Máquina no encontrada'
