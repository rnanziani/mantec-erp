...
export const getCargoById = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('SELECT idcargo_14, cargo_14 FROM tbl_14_cargo WHERE idcargo_14 = $1', [id]);
...
export const createCargo = async (req, res) => {
    try {
        ...
        const existingCargo = await pool.query('SELECT idcargo_14 FROM tbl_14_cargo WHERE LOWER(cargo_14) = LOWER($1)', [cargo_14.trim()]);
        ...
        const result = await pool.query('INSERT INTO tbl_14_cargo (cargo_14) VALUES ($1) RETURNING *', [cargo_14.trim()]);
        res.status(201).json({
            success: true,
            data: result.rows[0],
...
export const updateCargo = async (req, res) => {
    try {
        ...
        const existingCargo = await pool.query('SELECT idcargo_14 FROM tbl_14_cargo WHERE LOWER(cargo_14) = LOWER($1) AND idcargo_14 != $2', [cargo_14.trim(), id]);
        if (existingCargo.rowCount && existingCargo.rowCount > 0) {
            res.status(400).json({
                success: false,
                error: 'Ya existe un cargo con ese nombre'
        ...
        const result = await pool.query('UPDATE tbl_14_cargo SET cargo_14 = $1 WHERE idcargo_14 = $2 RETURNING *', [cargo_14.trim(), id]);
        if (result.rowCount === 0) {
            res.status(404).json({
                success: false,
                error: 'Cargo no encontrado'
...
export const deleteCargo = async (req, res) => {
    try {
        ...
        const checkUsage = await pool.query('SELECT id_tecnico_21 FROM tbl_21_tecnico WHERE id_cargo_21 = $1 LIMIT 1', [id]);
        ...
        const result = await pool.query('DELETE FROM tbl_14_cargo WHERE idcargo_14 = $1 RETURNING idcargo_14', [id]);
        if (result.rowCount === 0) {
            res.status(404).json({
                success: false,
                error: 'Cargo no encontrado'
