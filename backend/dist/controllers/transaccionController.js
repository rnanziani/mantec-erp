...
export const createTransaccion = async (req, res) => {
    try {
        const { id_alternador_28, id_ubicacion_origen_28, id_ubicacion_destino_28, id_tipo_transaccion_28, id_tecnico_28, id_maquina_28, fecha_28, hora_28 } = req.body;
        // Validación
...
export const deleteTransaccion = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM tbl_28_transaccion WHERE id_transaccion_28 = $1 RETURNING id_transaccion_28', [id]);
        if (result.rowCount === 0) {
            const response = {
                success: false,
                error: 'Transacción no encontrada'
