-- Permisos etapa 2: entrega / estados / semáforo
INSERT INTO tbl_05_permiso (nombre_permiso_05, descripcion_05, orden_05)
SELECT v.nombre, v.descripcion, v.orden
FROM (VALUES
  ('MENU_REPUESTOS_DANADOS_ENTREGA',            'Entrega de repuestos dañados al proveedor', 9140),
  ('MENU_REPUESTOS_DANADOS_ESTADO_REPARACION',  'CRUD estados de reparación', 9150),
  ('MENU_REPUESTOS_DANADOS_SEMAFORO',           'CRUD semáforo de días de entrega', 9160)
...
WHERE LOWER(u.email) = LOWER('rnanziani@gmail.com')
  AND p.nombre_permiso_05 IN (
    'MENU_REPUESTOS_DANADOS_ENTREGA',
    'MENU_REPUESTOS_DANADOS_ESTADO_REPARACION',
    'MENU_REPUESTOS_DANADOS_SEMAFORO'
  )
  AND NOT EXISTS (
