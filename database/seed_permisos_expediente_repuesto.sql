-- Permisos tablero y reportes del expediente de repuesto dañado
-- En DBeaver: Alt+X. Idempotente. Asigna a niveles con MENU_REPUESTOS_DANADOS.

INSERT INTO tbl_05_permiso (nombre_permiso_05, descripcion_05, orden_05)
SELECT v.nombre, v.descripcion, v.orden
FROM (VALUES
  ('MENU_REPUESTOS_DANADOS_EXPEDIENTE',          'Tablero expediente de repuesto dañado', 9170),
  ('MENU_REPUESTOS_DANADOS_EXPEDIENTE_REPORTES', 'Reportes y garantía del expediente', 9175)
...
WHERE p.nombre_permiso_05 LIKE 'MENU_REPUESTOS_DANADOS_EXPEDIENTE%'
  AND NOT EXISTS (
    SELECT 1 FROM tbl_050_nivel_permiso np
    WHERE np.id_nivel_04 = 1 AND np.id_permiso_05 = p.id_permiso_05
  );
...
WHERE LOWER(u.email) = LOWER('rnanziani@gmail.com')
  AND p.nombre_permiso_05 LIKE 'MENU_REPUESTOS_DANADOS_EXPEDIENTE%'
  AND NOT EXISTS (
...
CROSS JOIN tbl_05_permiso p
WHERE po.nombre_permiso_05 = 'MENU_REPUESTOS_DANADOS'
  AND p.nombre_permiso_05 LIKE 'MENU_REPUESTOS_DANADOS_EXPEDIENTE%'
  AND NOT EXISTS (
